  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    await db.execute('''
      CREATE TABLE words(
        ...
        headword TEXT, -- Kelime tekrarını önlemek için UNIQUE eklendi
        pos TEXT,
        cefr TEXT,
        sentence TEXT,
        sentence_tr TEXT,
...
Future<void> _seedDatabase(Database db) async {
  try {
    ...
    print('Toplam ${data.length} kelime bulundu. Ekleme işlemi başlıyor (TEK BATCH)...');
    
    for (var item in data) {
      try {
        final Word word = Word.fromJson(item as Map<String, dynamic>).copyWith(id: null); 
    ...
    print('Veritabanı DOLDURMA BAŞARILI. Toplam ${data.length} kelimeden, $counter kelime ekleme komutuna eklendi. ($errorCount hata)');
    ...
    print('DB Kontrolü: Veritabanında şu an $count kelime var. Beklenen: 7799.');
